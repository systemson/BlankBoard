@if (session('success'))
  <div class="alert alert-success alert-dismissable fade in">
    ...
    <h3>{{ __('messages.alert.' . session('success')) }}</h3>
    @if (Lang::has('messages.alert.' . session('success') . '-small'))
      <p>{{ __('messages.alert.' . session('success') . '-small') }}</p>
    @endif
  </div>
@elseif (session('info'))
  <div class="alert alert-info alert-dismissable fade in">
    ...
    <h3>{{ __('messages.alert.' . session('info')) }}</h3>
    @if (Lang::has('messages.alert.' . session('info') . '-small'))
      <p>{{ __('messages.alert.' . session('info') . '-small') }}</p>
    @endif
  </div>
